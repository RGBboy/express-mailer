...
exports.extend = function (self, options) {
  ...
  self.sendEmail = function (template, locals, callback) {
    ...
    self.renderEmail(template, locals, function(err, html) {
  ...
  /**
  * Renders an email using the given template and locals;
  *
  * @param {String} template (the name of the template)
  * @param {Object} locals (template local variables)
  * @param {Function} callback
  * @api public
  */
  self.renderEmail = function(template, locals, callback) {
    self.render(template, locals, callback);
  };
